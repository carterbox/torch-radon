...
with torch.no_grad():
    ...
    print(x.shape, sinogram_parallel.shape, fbp_parallel.shape)
    print("Parallel-beam FBP error", torch.norm(x - fbp_parallel).item())

Image.fromarray(to_255_numpy(x)).save(os.path.join(output_dir, 'torchradonv2_x.png'))
Image.fromarray(to_255_numpy(sinogram_parallel)).save(os.path.join(output_dir, 'torchradonv2_sino_parallel.png'))
Image.fromarray(to_255_numpy(sinogram_filtered_parallel)).save(os.path.join(output_dir, 'torchradonv2_sino_filtered_parallel.png'))
Image.fromarray(to_255_numpy(fbp_parallel)).save(os.path.join(output_dir, 'torchradonv2_y_parallel.png'))
...
with torch.no_grad():
    ...
    print(x.shape, sinogram_fan.shape, fbp_fan.shape)
    print("Fan-beam FBP error", torch.norm(x - fbp_fan).item())
    
Image.fromarray(to_255_numpy(sinogram_fan)).save(os.path.join(output_dir, 'torchradonv2_sino_fan.png'))
Image.fromarray(to_255_numpy(sinogram_filtered_fan)).save(os.path.join(output_dir, 'torchradonv2_sino_filtered_fan.png'))
Image.fromarray(to_255_numpy(fbp_fan)).save(os.path.join(output_dir, 'torchradonv2_y_fan.png'))
