...
FFTConfig::FFTConfig(int dv, int r, int c)
  : device(dv)
  , rows(r)
  , cols(c)
{
...
bool
FFTConfig::operator==(const FFTConfig& o) const
{
  return device == o.device && rows == o.rows && cols == o.cols;
}

std::ostream&
operator<<(std::ostream& os, FFTConfig const& cfg)
{
  return os << "(device: " << cfg.device << ", rows: " << cfg.rows
            << ", cols: " << cfg.cols << ")";
}

FFTStructures::FFTStructures(const FFTConfig& _cfg)
  : cfg(_cfg)
{
...
bool
FFTStructures::matches(const FFTConfig& k) const
{
  return k == cfg;
}
    ...
    float* y)
{
  ...
  cufftSafeCall(
    cufftExecR2C(fft->forward_plan, (cufftReal*)x, (cufftComplex*)y));
}

void
iFFT(FFTCache& fft_cache,
     const float* x,
     int device,
     int rows,
     int cols,
