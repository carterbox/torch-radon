...
ProjectionCfg::ProjectionCfg(int dc_u,
                             float ds_u,
                             int dc_v,
                             float ds_v,
                             float sd,
...
ProjectionCfg::ProjectionCfg(const ProjectionCfg& src)
  : det_count_u(src.det_count_u)
  , det_spacing_u(src.det_spacing_u)
  , det_count_v(src.det_count_v)
  , det_spacing_v(src.det_spacing_v)
  , s_dist(src.s_dist)
  , d_dist(src.d_dist)
  , pitch(src.pitch)
  , initial_z(src.initial_z)
...
dim3
ExecCfg::get_block_dim() const
{
  return dim3(bx, by, bz);
}

dim3
ExecCfg::get_grid_size(int x, int y, int z) const
{
  return dim3(roundup_div(x, bx), roundup_div(y, by), roundup_div(z, bz));
}

int
ExecCfg::get_channels(int batch_size) const
{
