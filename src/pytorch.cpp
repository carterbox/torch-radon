...
torch::Tensor radon_forward(torch::Tensor x, torch::Tensor angles,
...
torch::Tensor radon_backward(torch::Tensor x, torch::Tensor angles,
                             TextureCache &tex_cache, const VolumeCfg &vol_cfg,
                             const ProjectionCfg &proj_cfg,
                             const ExecCfg &exec_cfg) {
  CHECK_INPUT(x);
...
void radon_add_noise(torch::Tensor x, RadonNoiseGenerator &noise_generator,
                     const float signal, const float density_normalization,
  ...
  noise_generator.add_noise(x.data_ptr<float>(), signal, density_normalization,
