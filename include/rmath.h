...
sin_poly(float x)
{
  constexpr float poly[6] = {
    -2.0366233e-8f, 2.6998228e-6f,  -1.980874e-4f,
    8.3324076e-3f,  -1.6666553e-1f, 9.999996e-1f,
  };
...
{
  constexpr float poly[6] = {
    0.0000430638f, 0.0002765672f, 0.0001520143f,
    0.0092705272f, 0.0422820123f, 0.0705230784f,
  };
...
exp(float x)
{
  ...
  // 2^floor(k/n) is a shift of -5 to divide by n and a shift of 52 to put
  // floor(k/n) into the exponent bits summing on the uint representation is
  // used to compute the product because the mantissa of 2^floor(k/n) is zero
  t += ki << (52 - 5);
...
// specify sorted = true when you are processing "long" sequences of
// positive/negative values. long sequences with same sign ===> few branch
// mispredictions ===> "if" is more perfomant
